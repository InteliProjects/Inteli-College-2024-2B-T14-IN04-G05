...
void setup() {
    ...
    if (!controleRFID.inicializar()) {
        ...
        while (1);  // Para o programa se o módulo RFID não inicializar
    }
...
void loop() {
    // Executa as rotinas do controle biométrico
    controleBiometrico.loop();
    
    // Executa as rotinas do gerenciador MQTT
    mqttManager.loop();
    
    // Executa as rotinas do controle RFID
    controleRFID.loop();
